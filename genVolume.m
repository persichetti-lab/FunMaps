...
for i = 1:length(roiNameArray)
    ...
    inFile = sprintf('%s/%s_consolidatedNets_%.3f.1D', roiClustDir,roiNameArray(i),roiThreshArray(i));
    maskFile = sprintf('%s/masks/%s_%imm.nii', homeDir, roiNameArray(i), roiDownDimArray(i));
    outFile = sprintf('%s/%s_consolidatedNets_%.3f.nii', homeDir, roiNameArray(i), roiThreshArray(i));
